...
def return_train_val_datasets(
    ...
    if name == "imagenet":
        train_dataset = ImagenetDataset(
            root=os.path.join(data_dir, 'data', 'imagenet', 'images', "train"),
            transform=train_transform,
        )
        val_dataset = ImagenetDataset(
            root=os.path.join(data_dir, 'data', 'imagenet', 'images', "val"),
...
def return_ood_loaders(
    ...
    return {
        "iNaturalist": tools.get_eval_loader(
            ImageFolder(
                root=os.path.join(data_dir, "images_largescale", "inaturalist"),
                transform=transform,
            )
        ),
        "SUN": tools.get_eval_loader(
            ImageFolder(
                root=os.path.join(data_dir, "images_largescale", "sun"),
                transform=transform,
            )
        ),
        "Places": tools.get_eval_loader(
            ImageFolder(
                root=os.path.join(data_dir, "images_largescale", "places"),
                transform=transform,
            )
        ),
        "OpenImage": tools.get_eval_loader(
            ImageFolder(
                root=os.path.join(data_dir, "images_largescale", "openimage_o"),
                transform=transform,
            )
        ),
        "Ninco": tools.get_eval_loader(
            ImageFolder(
                root=os.path.join(data_dir, "images_largescale", "ninco"),
                transform=transform,
            )
        ),
        "SSB-Hard": tools.get_eval_loader(
            ImageFolder(
                root=os.path.join(data_dir, "images_largescale", "ssb_hard"),
                transform=transform,
            )
        ),
        "Textures": tools.get_eval_loader(
            ImageFolder(
                root=os.path.join(data_dir, "images_classic", "texture"),
                transform=transform,
            )
        ),
    }
